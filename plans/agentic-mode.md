...
- **Prompt contract**: The console provides a platform-neutral prompt with absolute production resource URLs and always-visible source controls. It automatically selects a newly generated one-time-visible token, accepts a saved full token in browser-only state, and can restore `$SIGNMOS_TOKEN`.
...
- [ ] Public `/agent.md` and `/openapi.json` are accessible without auth, document the identity slice accurately, and contain no credential; the console prompt uses their absolute production URLs, automatically selects a newly generated token, accepts a saved full token in browser-only state, and can restore `$SIGNMOS_TOKEN` — [automated test: public artifacts, schema validation, and prompt UI]
...
- [ ] The platform-neutral console prompt uses absolute public guidance/OpenAPI URLs, has always-visible accessible token-source controls, automatically selects a new one-time-visible token, accepts a saved full token in browser-only state, and can restore `$SIGNMOS_TOKEN` — [automated test: console prompt and accessibility]
