...
- **Prompt contract**: The console provides a platform-neutral prompt with absolute production resource URLs. It references `$SIGNMOS_TOKEN` by default and substitutes only a newly generated, one-time-visible token after explicit user selection.
...
- [ ] Public `/agent.md` and `/openapi.json` are accessible without auth, document the identity slice accurately, and contain no credential; the console prompt uses their absolute production URLs, references `$SIGNMOS_TOKEN` by default, and substitutes only an explicitly selected one-time-visible token — [automated test: public artifacts, schema validation, and prompt UI]
...
- [ ] The platform-neutral console prompt uses absolute public guidance/OpenAPI URLs, references `$SIGNMOS_TOKEN` by default, substitutes only the explicitly selected one-time-visible token, and remains usable with accessible copy controls — [automated test: console prompt and accessibility]
